...
# Initialize LLM for idea generation
@st.cache_resource
def load_llm_model():
    """Load the TinyLlama model and tokenizer with caching for efficiency"""
    try:
        ...
        st.warning(f"Could not load TinyLlama model: {str(e)}. Will fall back to template-based ideas.")
        return None, None

# Generate research gap ideas using LLM
def generate_idea_with_llm(paper: Dict, topic: str, opportunity_keywords: List[Tuple[str, float]], 
                          model: Optional[Any], tokenizer: Optional[Any]) -> str:
    """Generate research gap ideas using TinyLlama model"""
    if model is None or tokenizer is None:
        # Fall back to template-based ideas if model isn't available
        return suggest_gap_idea_template(paper, topic, opportunity_keywords)
    
    # Extract key information from paper
    abstract = paper['abstract'][:300]  # Limit length to avoid token limits
    title = paper['title']
    
    # Create a prompt for the LLM
    keywords_text = ", ".join([k for k, _ in opportunity_keywords[:5]]) if opportunity_keywords else "N/A"
    
    prompt = f"""<|system|>
You are a research assistant helping identify novel research opportunities.
<|user|>
I'm researching "{topic}". I found a paper that stands out from the mainstream literature:

Title: {title}

Abstract excerpt: {abstract}

Trending keywords in outlier papers: {keywords_text}

Based on this information, suggest ONE specific, novel research gap or opportunity that I could pursue. Focus on cross-disciplinary approaches or unconventional methods. Be specific and insightful about what hasn't been explored yet.
<|assistant|>
"""
    ...
    try:
        ...
        st.warning(f"Error generating idea with LLM: {str(e)}. Falling back to template.")
        return suggest_gap_idea_template(paper, topic, opportunity_keywords)

# Template-based fallback for when LLM is unavailable
def suggest_gap_idea_template(paper, topic, opportunity_keywords):
    """Generate specific, relevant gap ideas based on templates (fallback method)."""
    abstract = paper['abstract'].lower()
    title = paper['title'].lower()
...
def save_embeddings(embeddings, papers, topic):
    ...
    filename = f"embeddings/{safe_topic}_{timestamp}"
    
    # Save the embeddings
    np.save(f"{filename}_embeddings.npy", embeddings)
    
    # Save paper metadata (without abstracts to save space)
    metadata = []
    for paper in papers:
        metadata.append({
            'title': paper['title'],
    ...
    pd.DataFrame(metadata).to_csv(f"{filename}_metadata.csv", index=False)
    return filename
...
def load_embeddings(filename):
    ...
    try:
        ...
        metadata = pd.read_csv(f"{filename}_metadata.csv")
...
def run_gap_finder():
    ...
    with st.sidebar:
        ...
        # LLM option
        st.divider()
        use_llm = st.checkbox("Use TinyLlama for Idea Generation", False,
                             help="Generate creative research gap ideas with LLM (requires more resources)")
        
        if use_llm:
            st.info("Loading TinyLlama model... this may take a moment on first run.")
            llm_model, tokenizer = load_llm_model()
            if llm_model is None:
                st.error("Could not load TinyLlama. Will use template-based ideas instead.")
                use_llm = False
        ...
        if use_llm:
            st.markdown("**LLM Mode Enabled:** Using TinyLlama to generate creative research gap ideas")
    ...
    if search_button:
        ...
        for i, paper in enumerate(gaps[:5], 1):
            ...
            with col2:
                # Research gap idea - using LLM or template
                st.markdown("#### Research Gap Idea")
                
                if use_llm:
                    with st.spinner("Generating idea with TinyLlama..."):
                        idea = generate_idea_with_llm(paper, topic, opportunity_keywords, llm_model, tokenizer)
                else:
                    idea = suggest_gap_idea_template(paper, topic, opportunity_keywords)
